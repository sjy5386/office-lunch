...
class Restaurant(Enum):
    ...
    정원정_한식뷔페 = (MenuSource.KAKAO_PLUS_FRIEND_POST, '_Zlxgxhxj', MenuFrequency.DAILY_LUNCH)
    한신IT타워_구내식당 = (MenuSource.KAKAO_PLUS_FRIEND_PROFILE_IMAGE, '_QRALxb', MenuFrequency.DAILY_LUNCH)
    ...
    def get_menu(self) -> Menu:
        try:
            if self.menu_source == MenuSource.INSTAGRAM_FEED:
                if self.menu_frequency in (MenuFrequency.DAILY_LUNCH, MenuFrequency.DAILY_DINNER):
